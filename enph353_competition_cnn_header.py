...
for i in range(NUM_IMAGES_TO_GENERATE):
  ...
  for j in range(1):
    subimg.append(img[20:80, 165:375])
    #print(subimg[-1])
    # cv2.imshow("test", np.array(subimg[i+j], dtype=np.uint8))
    # cv2.waitKey(0)
    #print(image_list[i%len(image_list)][1])
    #print(len(image_list[i%len(image_list)][1]))
    label_letter = image_list[i % len(image_list)][1][12]
    hot = int(label_letter)
    label.append(np.zeros(8, int).tolist())
    label[-1][hot] = 1
    print(label[-1])
...
history_conv = conv_model.fit(np.array(X_train_dataset), np.array(Y_train_dataset),
                              ...
                              epochs=200,
                              batch_size=8)
conv_model.save("CNN_headers.keras")
...
df_cm = pd.DataFrame(C_matrix, index = [i for i in "SVCTPMWB"],
                  columns = [i for i in "SVCTPMWB"])
